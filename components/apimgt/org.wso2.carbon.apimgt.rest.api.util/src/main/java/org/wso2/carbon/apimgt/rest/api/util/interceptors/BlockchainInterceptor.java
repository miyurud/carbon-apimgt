...
public class BlockchainInterceptor extends AbstractPhaseInterceptor {
    ...
    public void handleMessage(Message message) throws Fault {
        ...
        Message copyOfMessage = new MessageImpl();//message;//(Message)((Message)message.clone());
        MessageImpl.copyContent(message, copyOfMessage);

        InputStream is = copyOfMessage.getContent(InputStream.class);

        //InputStream isCopy = is.clone();

        //Message copy = (Message)((Message)message).clone();
        ...
        if(is!=null) {
            ...
            try {
                ...
                System.out.println("-------------------------------------------");
                bos.flush();
                //copyOfMessage.setContent(InputStream.class, is);
